<?php
  class Blog_Model extends CI_Model {
	 ...
	 public function getallcat($catid) {
          $sql = "SELECT *  from " . $this->db->dbprefix( 'tags' ) . " where tagid in (" . join(",", $catid) .") and status = '1' order by tagname asc";
	 ...
	 public function getall_blog($year=NULL,$month=NULL,$post_ids=NULL,$lastid=NULL) {
	  ...
	  public function getactivecat() {
          $sql = "SELECT tagid  from " . $this->db->dbprefix( 'post_tags' ) . " where status= '1' ";
	   ...
	   public function blog_byyear($year) {
	 if($year != ''){
		$sql = "SELECT * from " . $this->db->dbprefix( 'posts' ) . " where status= '1' and FROM_UNIXTIME(createdate, '%Y')= '" . $year .  "'";
	 }else{
		$sql = "SELECT *  from " . $this->db->dbprefix( 'posts' ) . " where status= '1'  group by  FROM_UNIXTIME(createdate, '%Y-%m') order by createdate desc " ;
	 }
