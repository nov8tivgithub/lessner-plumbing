<?php
  ...
  class Frontend extends CI_Controller {
	...
	$blogdata    = $this->blog_model->getall_blog($ymArray[0],$ymArray[1],$post_id);
	$categ		 =$this->blog_model->getallcat();
		...
		if ( !empty( $blogdata ) ) {
             foreach ( $blogdata as $cv => $cz ) {
			  ...
			  $moreBlogs = $this->blog_model->check_blog_more($lastid,$ymArray[0],$ymArray[1],$post_id);
          }
	...
	if(!empty($postdet)){
		...
		$loadblog    = $this->blog_model->getall_blog($ymArray[0],$ymArray[1],$post_id,$lastid);		
		$lastid = "";
		if ( !empty( $loadblog ) ) {
