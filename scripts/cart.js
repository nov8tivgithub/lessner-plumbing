...
function addnewshipping() {
	if (!$("#shippinginfo").hasClass("meera")) {
        ...
        //$("#shipmobile").val('');
        $("#shipstate").val('');
        $("#shipcountry").val('United States of America');
...
function changeshipping() {
	...
	$.post(ajaxurl + '/shippinglist', {
            ...
            if (data.success == '1') {
				...
				$("#shipmethoddiv").slideUp();
				$("#billingdata").slideUp();
				$("#paymentinfo").slideUp();
				$("#paymentreviewinfo").slideUp();
    ...
    $.post(ajaxurl + '/slectship', {
        ...
        if (data.success == '1') {
            if (data.shipkey) {
				...
				$("#cancelshippinginfo").html('<a class="crtitmrmvqt" onclick="cancelshippinginfo()">X</a>');
                $("#sameship").html('<a class="selctshpadrs sameships" onclick="sameasshipping(\'' + data.shipkey + '\')">Same as Shipping Address</a>')
                $("#shippinginfo").slideUp().removeClass("meeras");
            } else {
...
function addnewbilling() {
    ...
    if (!$("#billininfo").hasClass("meerash")) {
		...
		$("#billfname").val($("#billf").html());
        $("#billlname").val($("#billl").html());
        $("#billaddress").val($("#billa").html());
        $("#billcity").val($("#billc").html());
        $("#billzip").val($("#billz").html());
        $("#billemail").val($("#bille").html());
        $("#billphone").val($("#billp").html());
        
        $("#billcountry").val($("#billcon").html());
        
		if($("#billcon").html()!='United States of America'){
		 $("#state_otherbill").show();
		 $("#bill_state_other").val($("#bills").html());
		 $("#statebill").hide();
		}else{
		 $("#state_otherbill").hide();
		 $("#billstate").val($("#bills").html());
		 $("#statebill").show();
		}
		$("#paymentinfo").hide();
		$("#paymentreviewinfo").hide();
		...
		$("#addbill").hide();
		$("#billingdata").slideDown();
		$("#paymentinfo").show();
		$("#paymentreviewinfo").show();
  ...
  if($("#prsizeid_"+rowid).val()){
    ...
    $.post(ajaxurl + '/update_to_cart', {
        ...
        if (data.success == '1') {
			...
			else if(data.billkey && data.shipkey && data.shipping_method){
				$('#paymentreviewinfo').delay(0).fadeOut(1000);
		         $('#paymentinfo').delay(0).fadeIn(6000);
			}
            //$("#editcart").slideUp().removeClass("meera");
    ...
    if ($("#payment").valid()) {
        ...
        $.post(ajaxurl + '/paymentinfo', {
			...
			if(data.shiperror=='1'){
				...
				$("#billingdata").fadeTo( "slow", 1 );
				$("#shipmethoddiv").fadeTo( "slow", 1 );
				$("#payment").fadeTo( "slow", 1 );
				$("#paymentreviewinfo").fadeTo( "slow", 1 );	
			}
			if(data.billerror=='1'){
				alert(data.errormsg);
...
function selectshipping() {
   if($('input[name=shippingmethod]:checked', '#shippingmethod').val()){
	   var shipmethod = $('input[name=shippingmethod]:checked', '#shippingmethod').val();
	   var amnt = $("#amnt_"+shipmethod).val();
	   $("#paymentinfo").hide();
		$("#paymentreviewinfo").hide();
		$("#billininfo").hide();
		 var shpHTML = '<div id="shppreload" style="padding:35px;position:absolute;z-index:9999"><span class="prelod"></span></div>';
